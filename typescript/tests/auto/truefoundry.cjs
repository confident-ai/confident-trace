...
(async () => {
  ...
  const runtime = init({
    ...
    truefoundryProxyUrls: ['http://truefoundry.test/gateway'],
  });
  ...
  try {
    const client = new OpenAI({
      apiKey: 'test',
      baseURL: 'http://truefoundry.test/gateway',
    });
    ...
    for (const span of spans) {
      ...
      assert.equal(span.attributes['gen_ai.provider.name'], 'truefoundry');
      assert.equal(span.attributes['confident.gateway.name'], 'truefoundry');
    ...
    console.log('TrueFoundry CommonJS automatic/manual coexistence passed');
  } finally {
