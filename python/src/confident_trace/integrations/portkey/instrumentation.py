"""Instrument the native portkey SDK using its OpenAI-compatible payloads."""
...
TARGETS = [
    ("portkey_ai.api_resources.apis.chat_complete", c, "create")
    for c in ("Completions", "AsyncCompletions")
] + [
    ("portkey_ai.api_resources.apis.responses", c, "create")
...
def begin(params, instance=None):
    return begin_call(
        ...
        params,
        instance,
        lambda _: {ai.GEN_AI_PROVIDER_NAME: "portkey", SPAN_TYPE: "llm"},
        request,
        integration=Integration.PORTKEY,
    )
