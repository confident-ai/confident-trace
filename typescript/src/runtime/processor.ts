...
const CALL_RECORDING_PATH = '/v1/recordings';
...
export function createSpanProcessor(
  ...
  if (
    baseOptions?.protocol === 'http/protobuf' &&
    baseOptions.endpoint?.endsWith(SPAN_BATCH_PATH)
  )
    state.recordingUpload = {
      url:
        baseOptions.endpoint.slice(0, -SPAN_BATCH_PATH.length) +
        CALL_RECORDING_PATH,
